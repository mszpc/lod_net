...
from src.util import coco_eval, bbox2result_1image, results2json, get_seg_masks, eval_dict_iou
...
def lodnet_eval(dataset_path, ckpt_path, ann_file):
    ...
    for data in ds.create_dict_iterator(output_numpy=True, num_epochs=1):
        ...
        start = time.time()
        # run net
        output = net(Tensor(img_data), Tensor(img_metas), Tensor(gt_bboxes), Tensor(gt_labels),
                     Tensor(gt_num),
                     Tensor(gt_mask))
        end = time.time()
    ...
    result_files = results2json(dataset_coco, outputs, ckpt_path)
    dice_iou = eval_dict_iou(result_files, dataset_coco, config.coco_root, print_log)
    # all_summary_metrics = coco_eval(result_files, eval_types, dataset_coco, single_result=False)
...
def eval_():
    ...
    config.checkpoint_path = 'save_checkpoint/etis_base_5e4_0/etis_base_5e4-15_725.ckpt'
    print_log('================== start eval ==================')
    print_log(config.checkpoint_path)
    dice_iou = lodnet_eval(mindrecord_file, config.checkpoint_path, config.ann_file)
    print_log(json.dumps(dice_iou, indent=1))


# 修改 my_device_id、checkpoint_path、数据集位置
my_device_id = 7
...
def print_log(_str):
    print(_str)
    # 独立的写入文件
    f = open(config.checkpoint_path + '.log.txt', 'a+')
    f.write(_str + '\n')
    f.close()
