...
def eval_():
    ...
    config.checkpoint_path = 'save_checkpoint/base_lr1e3_0/base_lr1e3-12_2500.ckpt'
    print_log('================== start eval ==================')
    print_log(config.checkpoint_path)
    all_summary_metrics = maskrcnn_eval(mindrecord_file, config.checkpoint_path, config.ann_file)
    print_log(json.dumps(all_summary_metrics, indent=1))


# 修改 my_device_id、checkpoint_path、数据集位置
my_device_id = 7
# context.set_context(mode=context.GRAPH_MODE, device_target="Ascend", device_id=get_device_id())
context.set_context(mode=context.GRAPH_MODE, device_target="Ascend", device_id=my_device_id)
